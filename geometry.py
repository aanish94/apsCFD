...
class Geometry(object):
    ...
    def __init__(self, x_max, x_min=None, resolution=1000,
        ...
        self.y_vector_bottom = self.construct_y_vector(bounds_bottom, funcs_bottom, 0)

        # Rotate y vectors about the y-axis to complete the geometry by symmetry
    ...
    def construct_y_vector(self, bounds, funcs, default=0):
        ...
        # Create simple greater than / less than functions from the specified bounds
        conditional_list = []
        ...
        for idx, bound in enumerate(bounds):
            ...
            conditional_list.append((self.x_vector >= x_left) & (self.x_vector < x_right))

        # Evaluate piece-wise defined function for a given x array
        y_vector = np.piecewise(self.x_vector, conditional_list, funcs)
        
        if min(y_vector) < 0:
            raise ValueError("y-vector minimum : {} is below 0!".format(min(y_vector)))
    ...
    def visualize(self, file_name=None):
        ...
        ax.fill_between(self.x_vector, self.y_vector_top_reverse, self.y_vector_bottom_reverse,
                        facecolor='black')
